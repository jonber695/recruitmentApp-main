...
@TestExecutionListeners(listeners = { DependencyInjectionTestExecutionListener.class, HandleApplicationFormTest.class })
class HandleApplicationFormTest implements TestExecutionListener {
    @Autowired
        ...
        dbUtil.emptyDb();
           ...
           @Test
            void testApplicationForm(){
                HandleApplicationForm form = new HandleApplicationForm();
                form.setApplicantId(2);
                form.setNewStatus("accepted");
                Set<ConstraintViolation<HandleApplicationForm>> result = validator.validate(form);
